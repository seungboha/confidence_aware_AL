...
def get_metric_values(loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        list_correct = []
        list_logit = []
        total_num = 0    # total number of data points to be tested
        for inputs, targets, idx in loader:
            ...
            pred = output.data.max(1, keepdim=True)[1]    # predicted label

            total_acc += pred.eq(targets.data.view_as(pred)).sum()
        ...
        total_acc = 100. * total_acc / total_num
...
# By the author
def evaluate(data_loader, model, ce_criterion, n_query, epoch, logger, scoring=False):
    model.eval()
    ...
    with torch.no_grad():
        ...
        main_concat = torch.Tensor([]).cuda()
        target_concat = torch.Tensor([]).cuda()
        for num_batch_iter, (input, target, idx) in enumerate(data_loader):
            ...
            main_output = model(input)
            loss = ce_criterion(main_output, target)
            ...
            if scoring:
                main_concat = torch.cat((main_concat, main_output))
                target_concat = torch.cat((target_concat, deepcopy(target).float()))

            prec, correct = utils.accuracy(main_output, target)
            ...
            if num_batch_iter % 10 == 0:
                print('Test: [{0}][{1}/{2}]\t'
                    ...
                    n_query, num_batch_iter, len(data_loader),
                    batch_time=batch_time, loss=losses, top1=top1))

        # write the logger
        logger.write([n_query, epoch, top1.avg, losses.avg])

        if scoring:
            print('#                        done                         #')
            return top1.avg, losses.avg, main_concat, target_concat
        else:
